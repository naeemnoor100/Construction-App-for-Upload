...
export const InvoiceManager: React.FC = () => {
  ...
  const pagedInvoices = useMemo(() => filteredInvoices.slice(0, visibleCount), [filteredInvoices, visibleCount]);
  ...
  // Fix: Implemented handleOpenEditModal to populate form state for editing
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight uppercase">Billing Hub</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Tracking {invoices.length.toLocaleString()} issued bills.</p>
        </div>
        <button 
          onClick={() => { 
            // Fix: Reset form data when creating a new invoice
            setEditingInvoice(null); 
            setFormData({
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Receivables</p>
          <p className="text-2xl font-black text-amber-600">{formatCurrency(stats.receivable)}</p>
        </div>
      </div>

      <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="text" placeholder="Filter invoices..." className="w-full pl-12 pr-4 py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-sm font-bold outline-none" value={searchTerm} onChange={e => { setSearchTerm(e.target.value); setVisibleCount(INVOICES_PER_PAGE); }} /></div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                ...
                return (
                  <tr key={inv.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors">
                    ...
                    <td className="px-8 py-5">
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1"><Briefcase size={10} className="text-blue-500" /> {project?.name || 'N/A'}</p>
                    </td>
                    <td className="px-8 py-5">
                      <span className={`px-2.5 py-1 text-[9px] font-black uppercase rounded-lg border ${isPaid ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-600 border-red-100'}`}>{isPaid ? 'Paid' : 'Unpaid'}</span>
                    </td>
                    <td className="px-8 py-5 text-right font-black text-indigo-600 text-sm">{formatCurrency(inv.amount)}</td>
                    <td className="px-8 py-5 text-right">
                      <div className="flex justify-end gap-1">
                        <button onClick={() => handleOpenEditModal(inv)} className="p-2 text-slate-400 hover:text-indigo-600"><Pencil size={18} /></button>
        ...
        {visibleCount < filteredInvoices.length && (
          <div className="p-8 text-center bg-slate-50/30">
            <button onClick={() => setVisibleCount(p => p + INVOICES_PER_PAGE)} className="px-10 py-3 bg-white dark:bg-slate-800 border border-slate-200 rounded-2xl text-[10px] font-black uppercase shadow-sm">Load More Records ({filteredInvoices.length - visibleCount} remaining)</button>
          </div>
        )}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-800 rounded-[3rem] w-full max-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 border-b border-slate-100 dark:border-slate-700 bg-indigo-50/30 dark:bg-indigo-900/10 flex justify-between items-center">
               <h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">{editingInvoice ? 'Update Bill' : 'Generate Bill'}</h2>
               <button onClick={() => setShowModal(false)} className="p-2 text-slate-400"><X size={32} /></button>
            </div>
            <form onSubmit={handleSubmit} className="p-8 space-y-5">
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Project Site</label>
                <select className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 rounded-2xl font-bold dark:text-white outline-none appearance-none" value={formData.projectId} onChange={(e) => setFormData(p => ({ ...p, projectId: e.target.value }))} required>
                  <option value="" disabled>Choose site...</option>
                  {projects.filter(p => !p.isGodown).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                </select>
              </div>
              <textarea rows={2} className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 rounded-2xl font-bold dark:text-white outline-none" value={formData.description} onChange={(e) => setFormData(p => ({ ...p, description: e.target.value }))} placeholder="Invoice description..." required></textarea>
              ...
              <button type="submit" className="w-full bg-indigo-600 text-white py-5 rounded-[2rem] font-black uppercase shadow-2xl active:scale-95 transition-all text-sm tracking-widest">Authorize Invoice</button>
            </form>
          </div>
        </div>
      )}
